...
(() => {
  ...
  // Site is read-only for visitors: image browsing/dropping/reframe is an
  // admin-only (back-end) feature. Slots render their stored image only.
  const READONLY = true;
  ...
  const stylesheet =
    ...
    (READONLY ? '.empty,.ring,.ctl{display:none !important}:host{cursor:default}' : '');
  ...
  class ImageSlot extends HTMLElement {
    ...
    constructor() {
      ...
      if (!READONLY) this._empty.addEventListener('click', () => this._input.click());
      root.addEventListener('click', (e) => {
        ...
        if (READONLY || !this.hasAttribute('data-editable')) return;
        if (act === 'replace') {
      ...
      this.addEventListener('dblclick', (e) => {
        if (READONLY || !this.hasAttribute('data-editable') || !this._reframes()) return;
        e.preventDefault();
        if (this.hasAttribute('data-reframe')) this._exitReframe(true);
        else this._enterReframe();
      });
